...
const SkillsSection = ({
  ...
  const handleAddSkill = (type, skill, setSkill) => {
    ...
    const skillsArray =
      type === 'skillsToOffer' ? skillsToOffer : skillsToReceive
          ...
          {isEditing && (
            <div className="flex">
              <input
                ...
                className="flex-1 p-2 border border-gray-300 rounded-l-md focus:outline-none focus:ring-2 focus:ring-[#28a745]"
                onKeyPress={(e) => {
                  if (e.key === 'Enter') {
                    handleAddSkill(
                      'skillsToOffer',
              ...
              <button
                ...
                className="bg-[#28a745] text-white p-2 rounded-r-md hover:bg-[#218838] focus:outline-none"
              >
                <PlusIcon size={20} />
              </button>
            </div>
        ...
        <div>
          ...
          {isEditing && (
            <div className="flex">
              <input
                type="text"
                value={newReceiveSkill}
                onChange={(e) => setNewReceiveSkill(e.target.value)}
                placeholder="Add a skill..."
                className="flex-1 p-2 border border-gray-300 rounded-l-md focus:outline-none focus:ring-2 focus:ring-[#28a745]"
                onKeyPress={(e) => {
                  if (e.key === 'Enter') {
